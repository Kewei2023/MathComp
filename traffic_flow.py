...
root = './processed-speed-multi2'
save_dir = './processed-speed-multi2-flow'

os.makedirs(save_dir,exist_ok=True)
for ip_dir in os.listdir(root):

  if not os.path.isdir(os.path.join(root,ip_dir)): continue
  # if ip_dir != "32.31.250.103": continue
  ...
  for csv_file in os.listdir(os.path.join(root,ip_dir)):
    if 'csv' in csv_file:
      ...
      result_1 = df.groupby('time_ceil')[['all_speed','all_box_width','big_car_speed','big_car_box_width']].mean().reset_index()
