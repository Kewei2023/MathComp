...
for ip_dir in os.listdir(root):
    ...
    for csv_file in os.listdir(os.path.join(root, ip_dir)):
        if 'csv' in csv_file:
            ...
            # ipdb.set_trace()
            df['time_ceil'] = np.ceil(df['time(s)']*10).astype(int)
            # df['time(s)'] = df['time(s)'].round().astype(int)
            result_mean=df.groupby('time_ceil').mean().reset_index()
            result_sum = df.groupby('time_ceil').sum().reset_index()
            ...
            result_mean.to_csv(os.path.join(save_dir, ip_dir, csv_file.replace(".csv", "mean.csv")))
            result_sum.to_csv(os.path.join(save_dir, ip_dir, csv_file.replace(".csv", "sum.csv")))